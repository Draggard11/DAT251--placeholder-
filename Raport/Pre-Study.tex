...
To better understand the domain, we have followed the guidelines for design thinking\cite{DesignThinking},
\textit{empathizing} with our user base through conducting interviews with both students and teachers across a wide range of
educational levels and ages.
Through these interviews we captured perceived challenges related to learning, motivation, and engagement in educational settings.
These findings helped us in \textit{defining} our user base and pain points, and importantly, \textit{why} these are problems,
helping us place these insights within a broader context.

\subsection{Interview highlights}\label{pre-study-interviews}
...
\subsection{Building Personas}\label{building-personas}

Based on the interview data, three personas were developed to represent
key user groups and synthesize recurring patterns in experiences, motivations,
and challenges.These personas are included in the appendix and are used
as analytical tools throughout the project to ensure
...
\begin{figure}[H]
    ...
    \caption{This is a descriptive Domain model of the system showing relationships between students, courses, lecturers, and study groups.}
    \label{fig:domain-model}
\end{figure}

The domain model shown in Figure~\ref{fig:domain-model} is a descriptive model~\cite{Descriptive_vs_prescriptive}
that represents the key entities and relationships within the system.
It includes the entities \textit{Lecturer}, \textit{Course}, \textit{Enrollment},
\textit{Student}, \textit{StudyGroup}, and \textit{University}.
In the context of model-driven engineering, the model can be classified as descriptive,
meaning that it captures how the domain currently exists rather than
prescribing how the system should be implemented~\cite{Descriptive_vs_prescriptive}.

The model captures the structure of the academic environment based on insights derived from interviews and personas.
It reflects real-world relationships between students, lecturers, and courses, and is therefore useful for understanding
the problem domain and communicating with stakeholders.

At this stage of the project, a descriptive model is appropriate, as the goal is to explore and understand the domain
before committing to specific design or implementation decisions.
In later stages, the model could evolve into a more
\textit{prescriptive model}, guiding the system architecture and implementation.
...
In this pre-study we have followed the design thinking process \cite{DesignThinking}.
The focus has primarily been on the early stages of design-thinking, namely empathizing
with the target demographic and transitioning this understanding toward defining specific problem characteristics.
This phase ensured that the problem formulation was grounded in user needs,
reducing the risk of developing solutions based on assumptions rather than empirical insight.
...
To obtain a broad perspective on the problem domain, we aimed for diversity among interview participants.
...
Based on the insights gathered during the empathize and define phases, we proceeded to the ideation phase,
where we narrowed the focus to a specific target demographic and problem domain.
As a result from our group's brainstorming, we settled on focusing on university
students and improving motivation and social attendance through gamification.
...
This concludes the design-thinking activities relevant to the pre-study phase of this report.
Later on in the report we will further elaborate on our experiences with the cycles of prototyping and testing.
...
the group initially struggled with abstraction by prematurely committing to a concrete solution.
The initial idea was to develop a system that gamifies studying. 
After discussing this approach with the course instructor, it became clear that
the focus needed to shift away from implementation and toward understanding the problem domain.

As described by Kramer\cite{AbstractionKey}, poor abstraction often
leads to solutions that replicate complexity or address surface level
...
Semi-structured interviews were conducted to explore why students and teachers experience
...
Overall, this phase ensured that subsequent design decisions were grounded in empirically observed user needs rather than initial assumptions.
